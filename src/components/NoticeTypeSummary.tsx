...
export function NoticeTypeSummary({ certificates }: Props) {
  ...
  const groups = new Map<string, { inNotice: number; notificationSum: number }>();
  for (const c of certificates) {
    const name = c.objectType.name;
    const g = groups.get(name) ?? { inNotice: 0, notificationSum: 0 };
    g.inNotice += 1;
    g.notificationSum += c.notificationCount;
    groups.set(name, g);
  }
  ...
  return (
    <div className="mb-6">
      ...
      <ul className="flex flex-wrap gap-2">
        {entries.map(([name, { inNotice, notificationSum }]) => (
          <li
            key={name}
